...
| Entra | Custom Banned Password Count | ENTRA-PASSWORD-004 |
...
### CheckId Fix: ENTRA-PASSWORD-002 Split

The current code uses `ENTRA-PASSWORD-002` for two distinct checks:
- "Custom Banned Password List Enforced" (enforcement toggle)
- "Custom Banned Password Count" (list content)

Split into:
- `ENTRA-PASSWORD-002` — keeps "Custom Banned Password List Enforced"
- `ENTRA-PASSWORD-004` — new, for "Custom Banned Password Count"

## Severity Change Audit

> **Completeness note**: Only checks whose severity changes are listed below.
> All checks not listed retain their current status. This includes checks
> already at the correct severity (e.g., Defender Spoof Intelligence = Fail,
> Teams External Access checks = Fail, SharePoint sharing checks with existing
> nuanced logic, etc.).
...
| 7 | ENTRA-PASSWORD-004 | Custom Banned Password Count | Review | Warning | Empty list is determinable (split from PASSWORD-002) |
...
| 3 | SPO-SHARING-007 | Default Sharing Link Permission | Warning (fallback) | Keep Warning | Already Warning for 'Edit' case; fallback/default case also uses Warning |
...
- **Rewrite rule**: Every remediation string that starts with a UI path (e.g., "Entra admin center > ...") instead of a PowerShell command must be rewritten to command-first format. If no PowerShell command exists for the setting, the UI path is acceptable as-is.

## Files Modified

| File | Changes |
|------|---------|
| `Entra/Get-EntraSecurityConfig.ps1` | 8 severity changes, 19 remediation rewrites, 7 new CheckIds (incl. PASSWORD-004 split) |
| `Security/Get-DefenderSecurityConfig.ps1` | 15 severity changes, 31 remediation rewrites |
| `Collaboration/Get-SharePointSecurityConfig.ps1` | 1 severity change, 6 remediation additions, 3 new CheckIds |
| `Collaboration/Get-TeamsSecurityConfig.ps1` | 1 severity change, 3 remediation additions, 2 new CheckIds |
| `controls/registry.json` | 12 new entries with framework mappings (incl. PASSWORD-004) |
