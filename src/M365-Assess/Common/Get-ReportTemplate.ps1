...
$accentCss = if ($accentColor -or $primaryColor) {
    $vars = @()
    if ($accentColor)  { $vars += "--m365a-accent: $accentColor;" }
    if ($primaryColor) { $vars += "--m365a-primary: $primaryColor;" }
    "<style>:root { $($vars -join ' ') }</style>"
} else { '' }

# Client logo tag (hero position on white-label cover page)
$clientLogoImgTag = if ($clientLogoBase64) {
    "<img src='data:$clientLogoMime;base64,$clientLogoBase64' alt='Client Logo' class='wl-client-logo' />"
} elseif ($logoBase64) {
    "<img src='data:$logoMime;base64,$logoBase64' alt='$brandName' class='wl-client-logo' />"
} else {
    "<div class='wl-client-logo-text'>$(ConvertTo-HtmlSafe -Text $TenantName)</div>"
}

# Company logo tag (secondary "Prepared by" pill on white-label cover page)
$companyLogoImgTag = if ($WhiteLabel -and $logoBase64 -and $clientLogoBase64) {
    "<img src='data:$logoMime;base64,$logoBase64' alt='$(ConvertTo-HtmlSafe -Text $brandName)' class='wl-company-logo' />"
} else { '' }

# Cover display name ("Prepared For" — ClientName or fallback to TenantName)
$coverClientName = if ($clientName) { $clientName } else { $TenantName }

# Cover title text
$coverTitleText = if ($reportTitle) { $reportTitle } else { 'M365 Security Assessment Report' }

# Framework "assessed against" line for cover page
$frameworkAssessedLine = if ($WhiteLabel -and $frameworkDisplayLabels.Count -gt 0) {
    $labelsHtml = ($frameworkDisplayLabels | ForEach-Object { ConvertTo-HtmlSafe -Text $_ }) -join ' &amp; '
    "<div class='wl-framework-line'>Assessed against: $labelsHtml</div>"
} else { '' }

# Confidentiality bar (shown when disclaimer is set or -WhiteLabel without branding suppression)
$confidentialityBar = if ($disclaimer) {
    "<div class='wl-confidential'>&#9888; $(ConvertTo-HtmlSafe -Text $disclaimer)</div>"
} else { '' }

# Sidebar nav title — removes GitHub link in white-label mode
$navTitleHtml = if ($WhiteLabel) {
    $subtitle = if ($sidebarSubtitle) { "<span class='nav-subtitle'>$(ConvertTo-HtmlSafe -Text $sidebarSubtitle)</span>" } else { '' }
    "<span class='nav-title'>$(ConvertTo-HtmlSafe -Text $brandName)$subtitle</span>"
} else {
...
# Footer attribution line — stripped of M365 Assess identity in white-label mode
$footerLineHtml = if ($WhiteLabel) {
    $label = if ($footerText) { ConvertTo-HtmlSafe -Text $footerText } elseif ($brandName -ne 'M365 Assess') { ConvertTo-HtmlSafe -Text $brandName } else { 'Security Assessment Report' }
    if ($footerUrl) { "<a href='$(ConvertTo-HtmlSafe -Text $footerUrl)' target='_blank' rel='noopener'>$label</a> v$assessmentVersion" }
    else { "$label v$assessmentVersion" }
} else {
    "Generated by <a href='https://github.com/Galvnyz/M365-Assess' target='_blank' rel='noopener' class='m365a-name'>M365 Assess</a> v$assessmentVersion"
}
...
<head>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* ── White-label cover page ──────────────────────────── */
        .wl-cover { padding: 0; display: flex; flex-direction: column; min-height: 100vh; }
        .wl-cover-hero {
            display: flex; flex-direction: column; align-items: center;
            padding: 36px 32px 28px; gap: 12px; text-align: center;
        }
        .wl-client-logo { max-width: 240px; max-height: 80px; object-fit: contain; }
        .wl-client-logo-text { font-size: 22px; font-weight: 800; color: #fff; }
        .wl-cover-divider { width: 48px; height: 2px; background: rgba(255,255,255,0.4); border-radius: 2px; }
        .wl-cover-title { font-size: 22px; font-weight: 800; color: #fff; letter-spacing: -0.02em; line-height: 1.2; }
        .wl-cover-subtitle { font-size: 11px; color: rgba(255,255,255,0.6); letter-spacing: 0.07em; text-transform: uppercase; }
        .wl-prep-pill {
            display: inline-flex; align-items: center; gap: 7px;
            background: rgba(0,0,0,0.2); border: 1px solid rgba(255,255,255,0.15);
            border-radius: 20px; padding: 5px 12px 5px 8px; margin-top: 4px;
        }
        .wl-company-logo { height: 18px; width: auto; object-fit: contain; }
        .wl-pill-label { font-size: 9.5px; color: rgba(255,255,255,0.6); font-weight: 500; }
        .wl-pill-name { font-size: 10px; color: rgba(255,255,255,0.9); font-weight: 700; }
        .wl-cover-body { background: #fff; padding: 20px 26px; flex: 1; }
        .wl-field-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; margin-bottom: 12px; }
        .wl-field { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 7px; padding: 9px 12px; }
        .wl-field-label { font-size: 8.5px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.09em; color: #94a3b8; margin-bottom: 3px; }
        .wl-field-value { font-size: 12px; font-weight: 600; color: #1e293b; }
        .wl-field-muted { font-weight: 500; color: #475569; }
        .wl-report-note { background: #eff6ff; border-left: 3px solid #2563eb; border-radius: 0 6px 6px 0; padding: 8px 12px; margin-bottom: 10px; }
        .wl-report-note-label { font-size: 8px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.09em; color: #3b82f6; margin-bottom: 2px; }
        .wl-report-note-value { font-size: 12px; font-weight: 600; color: #1e40af; }
        .wl-framework-line { font-size: 10px; color: #64748b; margin-bottom: 8px; font-style: italic; }
        .wl-confidential {
            background: #fef9c3; border: 1px solid #fde68a; border-radius: 5px;
            padding: 7px 12px; font-size: 9.5px; color: #78350f; text-align: center; font-weight: 500;
        }

        /* Analyst Findings Narrative Card */
        .narrative-card { background: var(--m365a-card-bg); border: 1px solid var(--m365a-border); border-left: 4px solid var(--m365a-accent); border-radius: 8px; padding: 20px 24px; margin: 0 0 20px 0; }
        .narrative-card-header { display: flex; align-items: center; gap: 10px; margin-bottom: 14px; flex-wrap: wrap; }
        .narrative-icon { width: 20px; height: 20px; flex-shrink: 0; color: var(--m365a-accent); }
        .narrative-card-title { font-size: 12pt; font-weight: 700; color: var(--m365a-text); flex-shrink: 0; }
        .narrative-chips { display: flex; gap: 6px; flex-wrap: wrap; margin-left: auto; }
        .narrative-chip { display: inline-flex; align-items: center; padding: 3px 10px; border-radius: 12px; font-size: 0.78em; font-weight: 600; }
        .chip-critical { background: rgba(220,38,38,0.12); color: #b91c1c; }
        .chip-high     { background: rgba(234,88,12,0.12);  color: #c2410c; }
        .chip-medium   { background: rgba(217,119,6,0.12);  color: #b45309; }
        .chip-framework { background: rgba(99,102,241,0.12); color: #4f46e5; }
        .narrative-card-body { font-size: 10.5pt; line-height: 1.65; color: var(--m365a-text); }
        .narrative-card-body p { margin: 0 0 10px 0; }
...
if (-not $CompactReport) {
    if ($WhiteLabel) {
        ...
        $prepByPill = if ($companyLogoImgTag -or ($brandName -ne 'M365 Assess')) {
            $pillLogo = if ($companyLogoImgTag) { $companyLogoImgTag } else { '' }
            $pillName = if ($brandName -ne 'M365 Assess') { "<span class='wl-pill-name'>$(ConvertTo-HtmlSafe -Text $brandName)</span>" } else { '' }
            "<div class='wl-prep-pill'>$pillLogo<span class='wl-pill-label'>Prepared by</span>$pillName</div>"
        } else { '' }
        $reportNoteBlock = if ($reportNote) {
            "<div class='wl-report-note'><div class='wl-report-note-label'>Report Note</div><div class='wl-report-note-value'>$(ConvertTo-HtmlSafe -Text $reportNote)</div></div>"
        } else { '' }
        $html += @"

        <header class="cover-page cover-print-only wl-cover">
            <div class="wl-cover-hero" style="$coverBgStyle">
                $clientLogoImgTag
                <div class="wl-cover-divider"></div>
                <div class="wl-cover-title">$(ConvertTo-HtmlSafe -Text $coverTitleText)</div>
                <div class="wl-cover-subtitle">Microsoft 365 Configuration Review</div>
                $prepByPill
            </div>
            <div class="wl-cover-body">
                <div class="wl-field-grid">
                    <div class="wl-field"><div class="wl-field-label">Prepared For</div><div class="wl-field-value">$(ConvertTo-HtmlSafe -Text $coverClientName)</div></div>
                    <div class="wl-field"><div class="wl-field-label">Prepared By</div><div class="wl-field-value">$(if ($brandName -ne 'M365 Assess') { ConvertTo-HtmlSafe -Text $brandName } else { '&mdash;' })</div></div>
                    <div class="wl-field"><div class="wl-field-label">Assessment Date</div><div class="wl-field-value wl-field-muted">$assessmentDate</div></div>
                    <div class="wl-field"><div class="wl-field-label">M365 Tenant</div><div class="wl-field-value wl-field-muted">$(ConvertTo-HtmlSafe -Text $TenantName)</div></div>
                </div>
                $reportNoteBlock
                $frameworkAssessedLine
                $confidentialityBar
            </div>
        </header>
"@
    } else {
        # Standard cover page (existing design, preserved for non-WhiteLabel runs)
        $html += @"
...
if (-not $CompactReport) {
    ...
    # Findings narrative card — injected between cover and executive summary
    if ($findingsNarrativeHtml) {
        $criticalCount = @($AllResults | Where-Object { $_.Severity -eq 'Critical' -and $_.Status -eq 'Fail' }).Count
        $highCount     = @($AllResults | Where-Object { $_.Severity -eq 'High'     -and $_.Status -eq 'Fail' }).Count
        $mediumCount   = @($AllResults | Where-Object { $_.Severity -eq 'Medium'   -and $_.Status -eq 'Fail' }).Count
        $severityChips = ''
        if ($criticalCount -gt 0) { $severityChips += "<span class='narrative-chip chip-critical'>Critical: $criticalCount</span>" }
        if ($highCount     -gt 0) { $severityChips += "<span class='narrative-chip chip-high'>High: $highCount</span>" }
        if ($mediumCount   -gt 0) { $severityChips += "<span class='narrative-chip chip-medium'>Medium: $mediumCount</span>" }
        $frameworkChips = ''
        foreach ($fl in $frameworkDisplayLabels) {
            $frameworkChips += "<span class='narrative-chip chip-framework'>$(ConvertTo-HtmlSafe -Text $fl)</span>"
        }
        $html += @"
        ...
        <div class="narrative-card">
            <div class="narrative-card-header">
                <svg class="narrative-icon" viewBox="0 0 20 20" fill="currentColor"><path d="M4 4.5A2.5 2.5 0 0 1 6.5 2h7A2.5 2.5 0 0 1 16 4.5v11a2.5 2.5 0 0 1-2.5 2.5h-7A2.5 2.5 0 0 1 4 15.5v-11ZM6.5 3A1.5 1.5 0 0 0 5 4.5v11A1.5 1.5 0 0 0 6.5 17h7A1.5 1.5 0 0 0 15 15.5v-11A1.5 1.5 0 0 0 13.5 3h-7ZM7 7.5a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5Zm0 3a.5.5 0 0 1 .5-.5h5a.5.5 0 0 1 0 1h-5a.5.5 0 0 1-.5-.5Zm0 3a.5.5 0 0 1 .5-.5h3a.5.5 0 0 1 0 1h-3a.5.5 0 0 1-.5-.5Z"/></svg>
                <span class="narrative-card-title">Analyst Findings Summary</span>
                <div class="narrative-chips">$severityChips$frameworkChips</div>
            </div>
            <div class="narrative-card-body">$findingsNarrativeHtml</div>
        </div>
"@
    }
