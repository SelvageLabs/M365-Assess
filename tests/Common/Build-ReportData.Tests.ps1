Describe 'Build-ReportData' {
    ...
    # ------------------------------------------------------------------
    Context 'Get-CmmcLevelsFromControlId' {
        It 'returns an empty array for null or blank input' {
            (Get-CmmcLevelsFromControlId -ControlId '')   | Should -BeNullOrEmpty
            (Get-CmmcLevelsFromControlId -ControlId $null) | Should -BeNullOrEmpty
        }

        It 'extracts a single level from a single control ID' {
            $levels = Get-CmmcLevelsFromControlId -ControlId 'IA.L2-3.5.5'
            @($levels) | Should -Be @('L2')
        }

        It 'extracts multiple distinct levels from a semicolon-delimited list' {
            $levels = Get-CmmcLevelsFromControlId -ControlId 'AC.L1-B.1.I;AC.L2-3.1.1;AU.L2-3.3.3'
            @($levels) | Should -Be @('L1', 'L2')
        }

        It 'dedupes repeated levels across many controls' {
            $levels = Get-CmmcLevelsFromControlId -ControlId 'IA.L2-3.5.1;IA.L2-3.5.2;IA.L2-3.5.3'
            @($levels) | Should -Be @('L2')
        }

        It 'includes L3 when an L3 practice is present' {
            $levels = Get-CmmcLevelsFromControlId -ControlId 'CA.L2-3.12.1;RA.L3-3.11.1E;SI.L3-3.14.6E'
            @($levels) | Should -Be @('L2', 'L3')
        }

        It 'returns an empty array when no L<n>- token is present' {
            (Get-CmmcLevelsFromControlId -ControlId 'not-a-cmmc-id') | Should -BeNullOrEmpty
        }
    }

    # ------------------------------------------------------------------
    Context 'CMMC profile override (regression for L1==L2 bug)' {
        It 'overrides fwMeta.cmmc.profiles with levels derived from controlId, not the registry array' {
            # Registry reports [L1,L2] but controlId is L2-only; output should say L2-only.
            $f = New-Finding -CheckId 'ENTRA-ENTAPP-020.1' -Status 'Fail' -Frameworks @{
                cmmc = @{ controlId = 'IA.L2-3.5.5'; profiles = @('L1', 'L2') }
            }
            $d = ConvertFrom-ReportDataJson (Build-ReportDataJson -AllFindings @($f))
            @($d.findings[0].fwMeta.cmmc.profiles) | Should -Be @('L2')
        }

        It 'still lists L1 when controlId genuinely contains an L1 practice' {
            $f = New-Finding -Status 'Pass' -Frameworks @{
                cmmc = @{ controlId = 'AC.L1-B.1.I;AC.L2-3.1.1'; profiles = @('L1', 'L2') }
            }
            $d = ConvertFrom-ReportDataJson (Build-ReportDataJson -AllFindings @($f))
            @($d.findings[0].fwMeta.cmmc.profiles) | Should -Be @('L1', 'L2')
        }

        It 'falls back to the registry profiles array when controlId has no level tokens' {
            $f = New-Finding -Status 'Pass' -Frameworks @{
                cmmc = @{ controlId = 'no-level-marker'; profiles = @('L1', 'L2') }
            }
            $d = ConvertFrom-ReportDataJson (Build-ReportDataJson -AllFindings @($f))
            @($d.findings[0].fwMeta.cmmc.profiles) | Should -Be @('L1', 'L2')
        }

        It 'does not touch profiles on non-CMMC frameworks' {
            $f = New-Finding -Status 'Pass' -Frameworks @{
                'cis-m365-v6' = @{ controlId = 'IA.L2-3.5.5'; profiles = @('L1') }
            }
            $d = ConvertFrom-ReportDataJson (Build-ReportDataJson -AllFindings @($f))
            @($d.findings[0].fwMeta.'cis-m365-v6'.profiles) | Should -Be @('L1')
        }

        It 'produces asymmetric L1 vs L2 cmmcCoverage counts for a realistic mix' {
            # Three findings: one L1+L2, one L2-only, one L3 only.
            # L1 should count only the first; L2 counts the first two; L3 counts the third.
            $l1And2 = New-Finding -CheckId 'A.1' -Status 'Pass' -Frameworks @{
                cmmc = @{ controlId = 'AC.L1-B.1.I;AC.L2-3.1.1'; profiles = @('L1', 'L2') }
            }
            $l2Only = New-Finding -CheckId 'A.2' -Status 'Fail' -Frameworks @{
                cmmc = @{ controlId = 'IA.L2-3.5.5'; profiles = @('L1', 'L2') }
            }
            $l3Only = New-Finding -CheckId 'A.3' -Status 'Warning' -Frameworks @{
                cmmc = @{ controlId = 'RA.L3-3.11.1E'; profiles = @('L1', 'L2', 'L3') }
            }
            $d = ConvertFrom-ReportDataJson (Build-ReportDataJson -AllFindings @($l1And2, $l2Only, $l3Only))

            $d.cmmcCoverage.L1.total | Should -Be 1
            $d.cmmcCoverage.L2.total | Should -Be 2
            $d.cmmcCoverage.L3.total | Should -Be 1
            $d.cmmcCoverage.L1.total | Should -Not -Be $d.cmmcCoverage.L2.total
        }
    }
