...
- Clickable CMMC and CIS profile chips in the Framework Quilt expanded panel: clicking L1, L2, L3 (CMMC) or L1, L2, E3, E5 only (CIS) sets `filters.profile`, filters the findings table to checks whose `fwMeta[fw].profiles` matches that level, and auto-scrolls to the findings section. Single-select; click the same chip again to clear. Selected chips show a subtle accent ring (#730, #731)
